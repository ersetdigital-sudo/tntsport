      ...
      <div className="relative min-h-[360px] sm:min-h-[390px]">
        ...
        <div className="absolute inset-x-0 bottom-0 grid grid-cols-3 overflow-hidden rounded-2xl border border-black/[.06] bg-white/95 shadow-premium-md backdrop-blur dark:border-white/10 dark:bg-[#15151f] sm:rounded-3xl">
          {stats.slice(0, 3).map((stat, index) => {
            ...
            return <div key={stat.label} className={`flex items-center justify-center gap-2 px-2 py-4 text-gray-950 sm:gap-3 sm:px-4 sm:py-5 dark:text-white ${index < 2 ? "border-r border-black/[.08] dark:border-white/10" : ""}`}>
              ...
              <div className="min-w-0 text-center sm:text-left"><p className="text-lg font-extrabold leading-none tracking-tight text-primary sm:text-2xl">{stat.value}</p><p className="mt-1 text-[9px] font-bold uppercase tracking-[.04em] text-gray-950 dark:text-white sm:text-[11px]">{stat.label}</p></div>
            </div>;
          })}
        </div>
      </div>
